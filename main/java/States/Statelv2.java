...
public class Statelv2 extends GameState {
    private boolean isRunning = false;

    private int brickCols = 4;
    ...
    public Statelv2(StatesManager sm) {
        super(sm);
        isRunning = true;
        initGame();
    }
    ...
    private void initBricks() {
        for (int i = 0; i < brickRows; i++) {
            for (int j = 0; j < brickCols; j++) {
                bricks.add(new Brick(i * 150 + 110, j * 80 + 20, brickWidth, brickHeight));
            }
        }
    }
    ...
    private void setDashboardScore(){
        if(score > MenuState.Level2Score){
            MenuState.Level2Score = score;
        }
    }
}
